...
/** Binary download (templates, error workbooks). */
export async function apiDownloadBlob(path: string, retried = false): Promise<Blob> {
  const headers = new Headers();
  const token = getAccessToken();
  if (token) headers.set('Authorization', `Bearer ${token}`);
  ...
  if (res.status === 401 && !retried) {
    const ok = await tryRefresh();
    if (ok) return apiDownloadBlob(path, true);
  }
